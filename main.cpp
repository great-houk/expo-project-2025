...
int random_range(int min, int max) {
	return min + (myRandom() % (max - min + 1));
}

// Generate a random float between 0.0 and 1.0
float random_float() {
	return (float)myRandom() / m;
}
...
void draw_rect(char* pixel_buf,
	...
	for (int i = start_x; i < end_x; i++) {
		for (int j = start_y; j < end_y; j++) {
			...
			// This fourth byte has to exist for alignment reasons, can hold
			// whatever as it's never read
...
void create_bullet(float x, float y, float rotation) {
	...
	bullets[bullet_count].velocity_x = sinf(rad) * bullet_speed;
	bullets[bullet_count].velocity_y = -cosf(rad) * bullet_speed;
	bullets[bullet_count].active = true;
	bullets[bullet_count].lifetime = 240;  // Bullet will disappear after 240
										   // frames (about 4 seconds at 60 FPS)
			...
			if (distance < 10) {  // Collision detected
				...
				for (int attempts = 0; attempts < 10; attempts++) {
					...
					float player_distance =
						sqrtf(player_dx * player_dx + player_dy * player_dy);
...
void init_asteroid(char* pixel_buf,
				   float x,
				   float y,
				   int width,
				   int height,
...
void move_asteroid(char* pixel_buf, struct Asteroid* myAsteroid) {
	...
	draw_rect(pixel_buf, draw_x, draw_y, myAsteroid->width, myAsteroid->height,
			  86, 107, 114);
}
...
void draw_player(char* pixel_buf, float x, float y, float rotation) {
	...
	// Front point (nose)
	points[0][0] = draw_x + (int)(size * sinf(rad));
	points[0][1] = draw_y - (int)(size * cosf(rad));

	// Back left point
	points[1][0] = draw_x + (int)(size * sinf(rad + 2.61799f));	 // 150 degrees
	points[1][1] = draw_y - (int)(size * cosf(rad + 2.61799f));

	// Back right point
	points[2][0] = draw_x + (int)(size * sinf(rad - 2.61799f));	 // -150 degrees
	points[2][1] = draw_y - (int)(size * cosf(rad - 2.61799f));
...
void draw_char(char* pixel_buf, int x, int y, char c, char r, char g, char b) {
	...
	switch (c) {
		...
		case '2':
			...
			draw_rect(pixel_buf, x + 4, y + 1, 1, 2, r, g,
					  b);  // Right vertical at top
			draw_rect(pixel_buf, x, y + 3, 5, 1, r, g, b);	// Middle horizontal
			draw_rect(pixel_buf, x, y + 4, 1, 2, r, g,
					  b);  // Left vertical at bottom
		...
		case '3':
			...
			break;
		case '4':
			draw_rect(pixel_buf, x, y, 1, 4, r, g,
					  b);  // Left vertical line (top portion)
			draw_rect(pixel_buf, x + 4, y, 1, 7, r, g,
					  b);  // Right vertical line (full height)
			draw_rect(pixel_buf, x, y + 3, 5, 1, r, g,
					  b);  // Middle horizontal line
			break;
		case '5':
			draw_rect(pixel_buf, x, y, 5, 1, r, g, b);
		...
		case 'S':
			draw_rect(pixel_buf, x, y, 5, 1, r, g, b);	// Top horizontal
			draw_rect(pixel_buf, x, y + 1, 1, 2, r, g,
					  b);  // Left vertical (top portion)
			draw_rect(pixel_buf, x, y + 3, 5, 1, r, g, b);	// Middle horizontal
			draw_rect(pixel_buf, x + 4, y + 4, 1, 2, r, g,
					  b);  // Right vertical (bottom portion)
			draw_rect(pixel_buf, x, y + 6, 5, 1, r, g, b);	// Bottom horizontal
			break;

		case 'c':
			// Current implementation shows a full box, but lowercase 'c' should
			// be open on the right top
			draw_rect(pixel_buf, x + 1, y, 4, 1, r, g,
					  b);  // Top horizontal (slightly indented)
			draw_rect(pixel_buf, x, y + 1, 1, 5, r, g, b);	// Left vertical
			draw_rect(pixel_buf, x + 1, y + 6, 4, 1, r, g,
					  b);  // Bottom horizontal
			draw_rect(pixel_buf, x + 4, y + 1, 1, 1, r, g,
					  b);  // Small top-right mark
			draw_rect(pixel_buf, x + 4, y + 5, 1, 1, r, g,
					  b);  // Small bottom-right mark
			break;
		...
		case 'o':
			...
			draw_rect(pixel_buf, x, y + 1, 1, 5, r, g, b);	// Left vertical
			draw_rect(pixel_buf, x + 1, y + 6, 3, 1, r, g,
					  b);  // Bottom horizontal
			draw_rect(pixel_buf, x + 4, y + 1, 1, 5, r, g,
					  b);  // Right vertical
			break;

		case 'r':
			// 'r' should have a stem and a hook at the top right
			draw_rect(pixel_buf, x, y, 1, 7, r, g,
					  b);  // Left vertical (full height)
			draw_rect(pixel_buf, x + 1, y, 3, 1, r, g, b);	// Top horizontal
			draw_rect(pixel_buf, x + 4, y + 1, 1, 2, r, g,
					  b);  // Right vertical (small hook)
			break;
		...
		case 'e':
			...
			draw_rect(pixel_buf, x + 1, y, 3, 1, r, g, b);	// Top horizontal
			draw_rect(pixel_buf, x, y + 1, 1, 5, r, g, b);	// Left vertical
			draw_rect(pixel_buf, x + 1, y + 3, 3, 1, r, g,
					  b);  // Middle horizontal
			draw_rect(pixel_buf, x + 1, y + 6, 3, 1, r, g,
					  b);  // Bottom horizontal
			draw_rect(pixel_buf, x + 4, y + 1, 1, 2, r, g,
					  b);  // Right vertical (top portion)
			draw_rect(pixel_buf, x + 4, y + 4, 1, 2, r, g,
					  b);  // Right vertical (bottom portion)
			break;
...
void init(char* pixel_buf, int* ind) {
	...
	for (int i = 0; i < MAX_ASTEROIDS; i++) {
		...
		init_asteroid(pixel_buf, rand_x, rand_y, 10, 10, rand_speed_x,
					  rand_speed_y);
	}
...
EVENTS control_handler(EVENTS* key_events) {
	...
	for (int i = 0; i < CONTROLLER_MESSAGE_LENGTH; ++i) {
		buffer[i] = (char)multicore_fifo_pop_blocking();
	}
...
void update(char* pixel_buf, int* ind, struct AppContext* app) {
	...
	if (game_over) {
		...
		draw_text(pixel_buf, WIDTH / 2 - 40, HEIGHT / 2 - 10, "GAME OVER", 127,
				  0, 0);
	...
	for (int i = 0; i < asteroid_count; i++) {
		...
		if (!player.invulnerable) {
			...
			if (distance < 10) {  // Simple collision detection
				...
				// Break out of the loop to prevent multiple collisions in the
				// same frame
				break;
			}
		}
	...
	for (int i = 0; i < player.lives; i++) {
		int life_rect_x =
			WIDTH - (i + 1) * (life_rect_size + life_rect_spacing);
