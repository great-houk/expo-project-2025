...
int main() {
    ...
    ********************************************************/
    ...
    int ret = cyw43_arch_wifi_connect_timeout_ms(WIFI_SSID, WIFI_PASSWORD, CYW43_AUTH_WPA2_AES_PSK, 60000);
    if (ret != 0) {
    ...
    /********************************************************
    *****           Grab IP and Set UDP socket          *****
    ********************************************************/
    struct netif *sta_netif = cyw43_arch_get_sta_netif();
    if (sta_netif) {
    ...
    ********************************************************/
    ...
    while (true) {
        ...
        // Move the two lines above outside the loop once we know the data is sent properly 


        printf("Waiting for a request from the client...\n");

        // Wait for a UDP packet from a client.
        ...
        buffer[recv_len] = '\0';  //Remove once we know the length of the message for sure


        printf("Client Request: %s\n", buffer);
        printf("From client: %s:%d\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));

        for (int i = 0; i < (CONTROLLER_MESSAGE_LENGTH + 1); ++i) {
            multicore_fifo_push_blocking((uint32_t)buffer[i]);
        }
    ...
    // Clean up: close the socket and deinitialize the CYW43 driver.
    close(sockfd);
    cyw43_arch_deinit();
    return 0;
}
